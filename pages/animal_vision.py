...
class AnimalVision:
    ...
    def print_matrix(self):
        return str(
            pd.DataFrame(
                self.M_rgb_to_cone, index=self.cone_names, columns=["R", "G", "B"]
            )
        )
    ...
    def render_image(
        ...
        if (self.sigma > 0.0) and use_blur:
            w, h = image.size
            radius = min(w, h) * self.sigma
            out = gaussian_filter(out, (radius, radius, 0.0))
            # result.filter(ImageFilter.GaussianBlur(radius=radius))

        return Image.fromarray((np.clip(out, 0, 1) * 255).astype(np.uint8))


st.set_page_config(page_title="Animal vision", page_icon="🐕")

st.title("Animal vision simulator")
...
if av is not None:
    ...
    st.write("錐体の吸収スペクトル (" + model + ", Govardovskii approx.)")
    av.plot_cones(ax=ax)
    st.pyplot(fig)
    st.write("Constructed RGB -> Cone Matix")
    st.write(av.print_matrix())
...
if av is not None and uploaded_file is not None:
    use_blur = st.checkbox("ガウスぼかし")

    image = Image.open(uploaded_file)
    preview = image.copy()
    preview.thumbnail((512, 512))

    result = av.render_image(preview, use_blur=use_blur)
